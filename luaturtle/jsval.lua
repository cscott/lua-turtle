...
StringMT.__tostring = JsValMT.__tostring -- actually defined lower down
SymbolMT.__tostring = JsValMT.__tostring
function NumberMT:__tostring() return tostring(self.value) end
BigIntMT.__tostring = JsValMT.__tostring
ObjectMT.__tostring = JsValMT.__tostring
...
function ObjectMT.ToPrimitive(env, input, hint)
   ...
   if env == nil then env = input[DEFAULTENV] end -- support for lua interop
...
function ObjectMT.OrdinarySetWithOwnDescriptor(env, O, P, V, Receiver, ownDesc)
   ...
   if ownDesc:IsDataDescriptor() then
      ...
      if existingDescriptor ~= nil then
         ...
         return mt(env, Receiver, 'CreateDataProperty', P, V)
      end
   end
...
function ObjectMT.OrdinarySetPrototypeOf(env, O, V)
   ...
   local current = O[PROTOTYPE]
   if V == current then return true end
   if O[EXTENSIBLE] == false then return false end
   local p = V
   local done = false
   while done == false do
      if p == Null then
...
function ObjectMT.ValidateAndApplyPropertyDescriptor(env, O, P, extensible, desc, current)
      ...
      if desc:IsGenericDescriptor() or desc:IsDataDescriptor() then
         if field ~= nil then
            if desc:IsSimple(false) then
               O[field] = desc.value or Undefined
            else
               O[field] = PropertyDescriptor:newData(desc)
            end
         end
      else
         if field ~= nil then
            O[field] = PropertyDescriptor:newAccessor(desc)
         end
      end
      return true
   end
   ...
   elseif current:IsDataDescriptor() ~= desc:IsDataDescriptor() then
      ...
      if current:IsDataDescriptor() then
         if field ~= nil then
            O[field] = PropertyDescriptor:newAccessor(current)
         end
      else
         if field ~= nil then
            O[field] = PropertyDescriptor:newData(current)
         end
      end
...
function JsValMT.__add(lval, rval, env) -- note optional env!
   ...
   local rnum = mt(env, rprim, 'ToNumeric')
   return lnum + rnum
end
